...
- In small data frames, we can diagnose cleanining operations easier than large data frames. 
- For example, in the large data frame, before cleaninig the sex variable with a data dictionary, we need to remove unconsistent missing values. We can use `dplyr::count()` to find this issue.
...
You can use [{epikinetics}](https://seroanalytics.org/epikinetics/) to estimate Bayesian hierarchical modelling of antibody kinetics. Explore this sample code:
