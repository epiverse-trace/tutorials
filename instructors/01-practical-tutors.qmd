...
- Second, complete the cleaning process. Connect these functions using pipe %>%:
...
- In the validation step, Do you need to allow for extra variable names and types for the date and categorical variables? _[Read this GitHub issue as a hint](https://github.com/epiverse-trace/linelist/issues/176) to allow for extra variables._
- What is the most apprioriate time unit to aggregate the incidence plot, based on visual inspection? 
- Does keeping or dropping arguments like `fill`, `show_cases`, `angle`, `n_breaks` improve the incidence plot? _[Read `plot()` reference manual](https://www.reconverse.org/incidence2/manual.html#sec:man-plot.incidence2) to find its arguments._
