...
For `{EpiNow2}`, we can specify it as a probability `distribution` adding its `mean`, standard deviation (`sd`), and maximum value (`max`). To specify a generation time that follows a _Gamma_ distribution with mean $\mu = 3.6$, standard deviation $\sigma^2 = 3.1$, and a maximum value of 20, we write:
...
In the `epinow()` function we can input three main elements:
