...
Finally, interventions such as vaccination may be of interest. A model may or may not have the capability to predict the impact of different interventions on different time scales (continuous time or at discrete time points). We will discuss interventions in detail in the next tutorial.

### Available  models
 
The R package `epidemics` contains functions to run existing models.
To find out what models are available see the package [vignettes](https://epiverse-trace.github.io/epidemics/articles). To learn more about each model read the documentation using `?epidemics::epidemic_ebola`. Recall the check model equation questions we can consider when trying to understand an existing model. 
